...
#[doc = "Field `LINE(0-18)` reader - Line %s status bit"]
...
#[doc = "Field `LINE(0-18)` writer - Line %s status bit"]
...
impl R {
    ...
    pub fn line_iter(&self) -> impl Iterator<Item = LINE_R> + '_ {
        (0..19).map(move |n| LINE_R::new(((self.bits >> n) & 1) != 0))
    }
