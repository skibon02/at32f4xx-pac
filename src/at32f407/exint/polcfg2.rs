...
#[doc = "Field `FP(0-19)` reader - Falling polarity configuration bit on line %s"]
...
#[doc = "Field `FP(0-19)` writer - Falling polarity configuration bit on line %s"]
...
impl R {
    ...
    pub fn fp_iter(&self) -> impl Iterator<Item = FP_R> + '_ {
        (0..20).map(move |n| FP_R::new(((self.bits >> n) & 1) != 0))
    }
